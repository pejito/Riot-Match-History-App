...
class ChallengerLeaderboard extends StatefulWidget {
  @override
  _ChallengerLeaderboardState createState() => _ChallengerLeaderboardState();
}

class _ChallengerLeaderboardState extends State<ChallengerLeaderboard> {

  late List<HighEloProfile> _players;

  Future<HighEloLeaderboard> _fetchLeaderboard(String auth) async{
    final response = await http.get(Uri.parse('https://na1.api.riotgames.com/tft/league/v1/challenger?api_key=' + auth));
    if(response.statusCode == 200){
      print("chal leaderboard status code:" + response.statusCode.toString());
              ...
              child: ListView.separated(
                  ...
                  itemBuilder: (context, index){
                    return Container(
                      ...
                      child: ListTile(
                        leading: Image.asset('assests/chal.png'),
                        title: Text(_players[index].summonerName, style: TextStyle(color: Colors.white)),
                        subtitle: Text(_players[index].lp.toString() + " LP", style: TextStyle(color: Colors.white),
                        ),
                      ),
                    );
                  },
